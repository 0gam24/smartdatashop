# D1 — Google 검색 SEO 종합 레퍼런스

> **원전**: developers.google.com/search/docs (Google Search Central 공식 문서)
> **참조 시점**: Google 검색 최적화 · 인덱싱 · 리치 결과 · Core Web Vitals
> **라이선스**: 원문 CC BY 4.0
...
| **301 리디렉트** | 영구 이전은 **반드시 서버 측 301** | old-domain.com → example.com 마이그레이션 |
...
확인: `site:example.com` 검색하여 결과가 나오면 인덱스에 포함된 것.
...
| 덕목 | 의미 | example.com 적용 |
|---|---|---|
| **Experience (경험)** | 직접 사용·방문·체험 | 1인 사업자 운영자(저자명) 본인 경험 기반 콘텐츠 |
...
- → **example.com 적용**: 모든 기사에 저자명 명시 + AuthorCard 컴포넌트

#### **어떻게** (How)
- 콘텐츠 제작 과정(테스트·조사·자동화 사용 여부)이 투명하게 공개되어 있는가?
- AI 또는 자동화를 사용한다면 어떻게 사용하는지 배경 제공
- → **example.com 적용**: AI 정책 페이지 + 각 글 하단 verifiedBy + previewMode 배너
...
- 외부 링크가 적은 신규 사이트
- 동영상·이미지·뉴스가 많은 사이트
- **Google News에 표시되길 원하는 사이트** ← example.com 해당
...
> **example.com 권장**: 일반 사이트맵 + 뉴스 사이트맵 (이미 `news-sitemap.xml` 보유). 자매 사이트는 카테고리에 따라 이미지 사이트맵 추가 검토.
...
✅ `https://example.com/policy/comprehensive-income-tax-guide`
❌ `https://example.com/2/6772756D707920636174` (의미 없는 식별자)
...
> **example.com 적용**: 폐기된 old-domain.com 계획 + old-domain.com Naver 차단 도메인 → example.com로의 마이그레이션 시점에 활용.
...
#### ~~`FAQPage`~~ — 🔴 **2026년 완전 폐지**
- **2026년 5월 발표 → 6월 15일 공식 문서 자체가 삭제됨.** FAQ 리치 결과는 **Google 검색결과에 더 이상 표시되지 않는다.**
  (`/search/docs/appearance/structured-data/faqpage` 는 현재 [최신 문서 업데이트](https://developers.google.com/search/updates?hl=ko) 페이지로 리다이렉트된다.)
- **이제 "일부 권위 사이트만 표시"가 아니라 전면 중단이다.** 2023년 8월 부분 축소 → 2026년 완전 종료.
- **조치**: 새 페이지에 `FAQPage` 마크업을 추가하지 않는다. 기존 마크업은 패널티 대상은 아니므로 급히 걷어낼 필요는 없으나,
  **리치 결과를 기대하고 FAQ 섹션을 억지로 넣는 설계는 중단한다.**
- FAQ 콘텐츠 자체는 사용자에게 유용하면 계속 써도 된다. 다만 **검색 노출 목적이 아니라 독자 목적**으로만 판단한다.
- 네이버는 별개다 — 네이버 지원 스키마는 [H/08](../H-네이버-공식가이드/08-구조화된%20데이터%20마크업/01-구조화된%20데이터%20소개.md)에서 확인한다.

### 12.3 단계적 중단된 타입

| 타입 | 상태 |
|---|---|
| **FAQPage** | 🔴 **2026-06 전면 폐지** (문서 삭제) |
| **연습 문제(Practice problems)** | 🔴 폐지 (문서 삭제) |
| **HowTo** | 폐지 — 일반 웹 검색결과에 미표시 |
| **사실 확인(ClaimReview)** | 일부 승인 매체만 |

> 현재 Google이 **실제로 지원하는 타입의 정답 목록**은 항상
> [search-gallery](../G-구글-공식가이드/05-structured-data-구조화데이터/search-gallery.md)에서 확인한다.
> 이 표는 요약이며, 지원 목록은 계속 줄어드는 추세다.
...
```html
<title>2026년 종합소득세 신고 가이드 — 사이트명</title>
```
