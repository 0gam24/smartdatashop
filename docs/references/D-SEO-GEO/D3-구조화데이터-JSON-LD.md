...
### 3-3. ~~FAQPage~~ 🔴 Google 리치 결과 폐지 (2026-06)

> **Google 검색에서는 더 이상 효과가 없다.** 2026년 5월 발표 → 6월 15일 공식 문서 삭제.
> Google은 [ai-optimization-guide](../G-구글-공식가이드/02-fundamentals-기본/ai-optimization-guide.md)에서
> **"생성형 AI 검색에 구조화된 데이터가 필요하지 않으며 추가해야 하는 특별한 schema.org 마크업도 없다"**고 명시했다.
> 따라서 아래 "AI 답변 엔진 인용에 유용" 근거는 **Google에 대해서는 성립하지 않는다.**
>
> - **신규 페이지**: `FAQPage` 추가하지 않는다.
> - **기존 페이지**: 패널티는 없으므로 급히 제거할 필요 없음. 다만 신규 설계 근거로 쓰지 않는다.
> - **네이버**: 별개 판단. 네이버 지원 목록은 [H/08](../H-네이버-공식가이드/08-구조화된%20데이터%20마크업/01-구조화된%20데이터%20소개.md) 확인.
> - **Google 지원 타입 정답**: [search-gallery](../G-구글-공식가이드/05-structured-data-구조화데이터/search-gallery.md)
>
> 아래 코드는 **이력 참고용**으로 남긴다.
...
→ **2026-06 정정**: FAQ 리치 결과 전면 폐지. Google 공식 문서도 삭제됐다. Google 대상 근거로 쓰지 않는다.
