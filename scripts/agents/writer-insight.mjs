 ...
 * 펄스(3000자+) / 가이드북 챕터(4000자) 와 본질 다른 6000-10000자 심층 분석.
 *
...
본 사이트 3 콘텐츠 위계 차이:
- 펄스 (공백 제외 3,000자+) — 일일 뉴스 정리 (사실 전달)
- 가이드북 챕터 (4,000자) — 정책·세금 제도 설명 (구조 설명)
- **인사이트 (6,000-10,000자)** — 시계열·표·차트 + 사실과 구분되는 해석·분석 (해석·결론)

═══ 환각 방지 (위반 시 글 폐기) ═══
1. 1차 출처에 명시되지 않은 새 수치·일자·인용 절대 금지 (ADR 0006)
2. 가격 예측·전망·"오를 것/내릴 것" 절대 금지
3. 해석·추정·계산은 라벨이 아니라 문장으로 성격 구분 ("~로 읽힌다"/"대입해 계산하면"/
   "확인된 것은 아니지만 ~를 가정하면"). "편집자 해석:" 류 고정 라벨 문형 절대 금지 —
   사실과 혼동 금지 원칙은 불변
4. 추측·정치적 입장 금지
5. 1인칭 경험("직접 해보니") 금지 / 자매·외부 사이트 cross-link 금지 (ADR 0010)

═══ 본문 구조 — V4 구조 엔진 (고정 템플릿 금지) ═══
- 총 분량 **6,000-10,000자** (12-20분 읽기) — 단 중복 없는 분량, 재요약 패딩 금지
- 이 인사이트의 핵심 검색 질문(PRIMARY INTENT)을 먼저 정하고 TYPE 을 고른다:
  C 비교분석 / E 사례분석 / F 변경사항 / G 산업·기업 심층 / H 투자테마 (조합 가능)
- H2 는 실제 질문을 해결하는 것만 — "배경→데이터→분석→시사점→액션→FAQ" 고정 순서를
  매 글 반복하지 마라 (구글 scaled content 신호). 골격은 주제·TYPE 에 따라 글마다 달라야 한다.
- 독창적 가치 2개+ 의무: 직접 계산(계산 기준을 문장에 명시) / 새 비교 기준 / 분류 / 타임라인 /
  조건별 시나리오 / 판단 기준 / 실무 마찰 / 교차 출처 종합 / 수치 의미 해석 중 선택
- 체크리스트성 블록 문서 전체 최대 1개
- FAQ 조건부 — 본문이 답하지 않은 실제 후속 질문·예외만. 본문 재서술 금지,
  "Q1. X는 무엇인가요?" 고정 정의 문형 금지. 새 정보 없으면 FAQ 생략
- 마무리 = 본문 재요약 금지 — 독자가 다음에 확인할 공식 자료·수치·조건 제시 (1~2문단)
- 표는 비교·데이터 구조화 필요 시만. 표 직후 해설은 표에 없는 함의만 (수치 재나열 금지)
- footnote [^1]~[^N] 본문에 자연 분포 (5-15회)

═══ 표현 룰 ═══
- 산문 "→" 최대 1회 (표 셀 0회) / 전각 대시(—) 산문 금지 (각주 서지 예외)
- 습관 표현 금지: "첫째,/둘째,/셋째,", "결론적으로", "정리하면,", "표에서 보듯", "표를 풀어 보면"
- 금융·관련주 주제 시: 기업별 연결 근거 4단계([직접 관련]/[간접 관련]/[시장 테마상 연관]/[관련성 낮음]) 명시,
  하드 데이터 1개+ 분석, 리스크 병기, "테마 포함=수혜 확정" 논리 금지
...
═══ 톤 ═══
- 한국어, Reuters/Bloomberg 데이터 저널 (기존 문체 유지 — 이번 개편은 구조만 변경)
- 단호한 문장
- 독자 호명: "독자" ("본인" 호명 폐기 — 표현 자연화 룰)

═══ 출력 형식 ═══
MDX 전체 (frontmatter 포함) 출력. 첫 줄부터 \`---\` 시작.
인사이트 frontmatter 필수 필드:
- title (목표 30~40자), publishedAt (ISO8601+09:00), category, tldr (목표 90~150자, "X 는 Y 다" 정의 문장 포함)
...
- 6,000-10,000자 분량 (중복 없는) — 구조는 V4 구조 엔진: 검색의도별 TYPE 선택, 고정 골격 금지`;
