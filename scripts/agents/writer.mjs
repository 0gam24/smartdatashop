...
4. 독창적 가치 2개 이상 (1차 출처 사실 범위 내에서): 직접 계산(계산 기준·과정을 문장에 명시),
...
9. 해석·추정·계산의 성격은 라벨이 아니라 문장으로 구분: 계산 = "대입해 계산하면/환산하면",
   해석 = "~로 읽힌다/~인 셈이다", 추정 = "확인된 것은 아니지만 ~를 가정하면".
   "편집자 계산:"/"편집자 해석:" 같은 고정 라벨 문형 절대 금지 (템플릿 신호 — 게이트 차단).
   사실과 혼동 금지 원칙은 불변 — 자체 계산이 공식 수치로 보이지 않게 기준을 문장에 명시.
