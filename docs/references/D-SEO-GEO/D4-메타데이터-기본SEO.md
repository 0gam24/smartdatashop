...
페이지별 메타 태그, 사이트 전체 파일(robots.txt, sitemap.xml, llms.txt=선택). PageSpeed Insights SEO 카테고리 100점의 기본.
...
## 5. llms.txt — AI 크롤러용 사이트 요약 (선택)

> 🔴 **Google 공식 입장 (2026-06-15 추가, [ai-optimization-guide](../G-구글-공식가이드/02-fundamentals-기본/ai-optimization-guide.md) 참조)**
>
> **"Google 검색은 llms.txt를 무시한다."** 생성형 AI 기능을 포함해 Google 검색 노출·순위에
> **긍정적으로도 부정적으로도 영향을 주지 않는다.** 새로운 기계 가독형 파일·AI 텍스트 파일·
> 특수 마크업을 만들 필요가 없다는 것이 Google의 명시적 답변이다.
>
> 다만 Google도 **"이 파일을 쓰는 다른 서비스나 시스템을 위해 만들어 두는 것은 완전히 괜찮다"**고 밝혔다.
>
> **→ 따라서 이 문서의 llms.txt는 Google용이 아니라 ChatGPT·Perplexity·Claude 등
> 타 AI 엔진 대비용 "선택 사항"으로만 취급한다. 필수 산출물이 아니며, Google SEO 점수와 무관하다.**
> 리소스가 부족하면 llms.txt보다 [유용한 콘텐츠 제작](../G-구글-공식가이드/02-fundamentals-기본/creating-helpful-content.md)과
> 기술 구조 정비에 시간을 쓰는 것이 Google 기준으로 항상 낫다.

### 5-1. 개요

- 위치: `/llms.txt` (사이트 루트)
- 형식: 마크다운
- 목적: **Google 외** AI 답변 엔진이 사이트 전체를 빠르게 이해하도록
- 우선순위: **선택** (Google 검색에는 무효)
...
- [ ] `llms.txt` 게시 (사이트 요약) — **선택. Google 검색은 무시하므로 필수 아님**
